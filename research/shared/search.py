...
The interrupter has no such parameter, and `lib.cipher` takes `skips` as a set
...
def beam_skips(
    ...
    depend on input <= i only. Every stream cipher in `lib.cipher` is; a
...
# sum; and `lib.cipher.apply_stream` makes the plaintext at position i depend on
